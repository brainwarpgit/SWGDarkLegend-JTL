...
//	}
	...
	int unitsExtracted = surveySkill * density * 4;//maxUnitsExtracted * (float(surveySkill) / 100.0f) * samplingMultiplier * cityMultiplier * 3;
//	unitsExtracted *= 5;
	int xpcap = 10;
...
//		trx.abort() << message.toString();
	...
	int xp = (int) (((float) unitsExtracted * 2.0 / (float) maxUnitsExtracted)
			* xpcap);
