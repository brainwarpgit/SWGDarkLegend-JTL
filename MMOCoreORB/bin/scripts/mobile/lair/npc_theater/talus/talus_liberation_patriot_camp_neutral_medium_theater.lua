talus_liberation_patriot_camp_neutral_medium_theater = Lair:new {
	mobiles = {{"mobiles=talus_liberation_party_patriot",15},{"mobiles=talus_liberation_party_patriot_elite",5},{"mobiles=talus_liberation_party_patriot_heroic",1}},
	spawnLimit = 12,
	buildingsVeryEasy = {"object/building/poi/corellia_selonian_separatists_small1.iff","object/building/poi/corellia_selonian_separatists_small2.iff"},
	buildingsEasy = {"object/building/poi/corellia_selonian_separatists_small1.iff","object/building/poi/corellia_selonian_separatists_small2.iff"},
	buildingsMedium = {"object/building/poi/corellia_selonian_separatists_small1.iff","object/building/poi/corellia_selonian_separatists_small2.iff"},
	buildingsHard = {"object/building/poi/corellia_selonian_separatists_small1.iff","object/building/poi/corellia_selonian_separatists_small2.iff"},
	buildingsVeryHard = {"object/building/poi/corellia_selonian_separatists_small1.iff","object/building/poi/corellia_selonian_separatists_small2.iff"},
	missionBuilding = "object/tangible/lair/base/objective_power_node.iff",
	mobType = "npc",
	buildingType = "theater"
}

addLairTemplate("talus_liberation_patriot_camp_neutral_medium_theater", talus_liberation_patriot_camp_neutral_medium_theater)
